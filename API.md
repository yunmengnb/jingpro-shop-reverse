...
浏览器不会直接请求店铺上游。后端将上游响应转换为字段白名单 DTO，并使用随机不透明 ID 托管支付、验证码和订单查询会话。
...
卡密解析兼容字符串、数组、JSON 字符串、账号/密码对象以及常见卡密字段。前端在已付款但卡密为空时每 3 秒自动刷新，最多 40 次。
