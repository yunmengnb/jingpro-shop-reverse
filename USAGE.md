...
9. 订单详情底部显示上游返回的“订单使用说明”；未配置时显示“暂无使用说明”。
10. “我的订单”使用当前登录手机号作为搜索条件，并通过图形验证码获得查询授权。
11. 点击“我的订单”中的历史订单时统一使用手机号查询授权，不依赖旧支付会话。

本地手机号保存在浏览器本地存储中；支付会话保存在浏览器会话存储和 Node.js 服务端内存中。清理浏览器数据或重启容器后，未完成的临时支付/查询授权可能失效。支付会话失效时，程序会清除旧 `payment_id` 并切换到当前手机号的图形验证码查询流程；订单查询授权失效时会要求重新完成验证码。
...
支付会话存放在服务端内存中，容器升级、重启或超过有效期后会失效。新版会清除浏览器中的旧 `payment_id`，然后按当前登录手机号重新进行图形验证码查询。若仍显示旧提示，请强制刷新页面并重新完成验证码。

### 订单使用说明为空

订单使用说明来自上游订单详情。上游商品或订单没有配置说明时，页面会显示“暂无使用说明”，程序不会用商品名称或卡密内容替代。
