...
- 商品分类、商品搜索、实时价格和支付渠道
- 本地手机号账号、用户管理弹窗、创建订单及支付状态查询
- 后端 Chromium 跟踪支付页面并快速截取最终收款二维码
- 图形验证码授权查询当前手机号的历史订单、卡密和订单使用说明
- 支付会话失效时自动切换至当前手机号的订单查询授权
...
1. 输入 `yes` 接受使用协议与免责声明
2. 完整店铺链接，例如 `https://shop.example.com/shop/demo`
3. 对外运行端口，默认 `3000`
4. 可选的绑定域名
...
- 上游 Cookie、店铺 Token、初始支付地址和验证码 ticket 只保存在服务端，不返回浏览器。
- 浏览器实际需要展示的商品、二维码、订单、卡密和使用说明仍可被当前用户查看。
- 服务端临时会话保存在内存中，升级或重启后会失效；历史订单会自动切换到手机号验证码授权。
