...
const state = { categories: [], goods: [], selected: null, channelId: 0, timer: null, orderDetailTimer: null, currentOrder: '', captcha: null, captchaResolve: null, account: '', orderAuthorization: null, orders: [], ordersLoading: null, paymentSessions: new Map(), goodsRequest: 0, channelsRequest: 0, priceRequest: 0, paymentRequest: 0, payment: null };
...
function removeOrderSession(orderNo) { state.paymentSessions.delete(String(orderNo)); sessionStorage.setItem(ORDER_SESSION_STORAGE_KEY, JSON.stringify(Object.fromEntries(state.paymentSessions))); const pending = pendingPayment(); if (pending?.orderNo === String(orderNo)) sessionStorage.removeItem(PENDING_PAYMENT_STORAGE_KEY); }
...
function applyAccount(phone) { state.account = phone; state.orderAuthorization = null; $('#contact').value = phone; $('#currentAccountPhone').textContent = phone; $('#accountModal').classList.add('hidden'); }
...
function logoutAccount() { localStorage.removeItem(ACCOUNT_STORAGE_KEY); clearOrderSessions(); state.account = ''; state.orderAuthorization = null; state.orders = []; $('#contact').value = ''; $('#currentAccountPhone').textContent = '—'; $('#userManagementModal').classList.add('hidden'); showAccountModal(); }
...
function showPaymentQr(data) {
  if (!/^data:image\/png;base64,[a-z0-9+/=]+$/i.test(data.qr_code || '')) throw new Error('未能截取最终支付页面二维码');
  const image = $('#paymentQrImage');
  image.src = data.qr_code;
  image.classList.remove('hidden');
  $('#paymentBrowserUrl').classList.add('hidden');
  $('#payStatus').textContent = '请使用对应支付应用扫描二维码完成付款';
}
async function openPayment(data) { const payment = paymentFields(data); if (!payment.orderNo || !payment.paymentId) throw new Error('支付会话创建失败'); state.payment = payment; $('#orderText').textContent = `订单号：${payment.orderNo}`; state.currentOrder = payment.orderNo; saveOrderSession(payment.orderNo, payment.paymentId); savePendingPayment(payment); saveOrder(payment.orderNo); $('#paymentModal').classList.remove('hidden'); $('#paymentQrImage').removeAttribute('src'); $('#paymentQrImage').classList.add('hidden'); $('#paymentBrowserUrl').classList.remove('hidden'); $('#paymentBrowserUrl').textContent = '服务器正在跟踪支付跳转并截取二维码…'; $('#payStatus').textContent = '正在跟踪支付平台跳转（约 3 秒）…'; pollOrder(payment.orderNo); try { const result = await api('payment-page', { payment_id: payment.paymentId }, 35000); showPaymentQr(result); } catch (error) { $('#paymentBrowserUrl').classList.remove('hidden'); $('#paymentBrowserUrl').textContent = error.message || '支付二维码获取失败，请关闭后重试'; $('#payStatus').textContent = '支付二维码加载失败'; throw error; } }
...
function pollOrder(orderNo) { clearTimeout(state.timer); const check = async () => { if ($('#paymentModal').classList.contains('hidden')) return; try { $('#payStatus').textContent = $('#paymentQrImage').classList.contains('hidden') ? '正在检测支付状态…' : '等待付款，正在自动检测支付状态…'; const data = await api('query', { trade_no: orderNo, payment_id: state.paymentSessions.get(String(orderNo)) || '' }); if (isPaid(data)) { state.timer = null; sessionStorage.removeItem(PENDING_PAYMENT_STORAGE_KEY); $('#payStatus').textContent = '支付成功，正在打开订单详情'; setTimeout(() => showOrderDetail(orderNo), 600); return; } } catch (error) { if (/支付会话.*失效/.test(error.message || '')) { removeOrderSession(orderNo); state.timer = null; $('#payStatus').textContent = '支付会话已更新，正在通过当前手机号查询订单…'; setTimeout(() => showOrderDetail(orderNo, '', 0, false), 600); return; } if (!$('#paymentModal').classList.contains('hidden')) $('#payStatus').textContent = '支付状态查询失败，3 秒后自动重试…'; } state.timer = setTimeout(check, 3000); }; check(); }
...
function renderOrdersList() { $('#ordersList').innerHTML = state.orders.length ? state.orders.map((item) => renderOrder({ trade_no: orderNo(item), result: { data: item } })).join('') : '<div class="empty-orders"><h2>暂无订单</h2><p>当前账号还没有历史订单。</p></div>'; }
async function authorizeCurrentOrders(force = false) { if (force) state.orderAuthorization = null; if (!state.orderAuthorization) { const captcha = await requestCaptcha(); if (!captcha) throw new Error('需要完成人机验证后查询订单'); state.orderAuthorization = { keywords: state.account, authorization_id: captcha.authorization_id }; } try { state.orders = unwrapList(await api('orders', state.orderAuthorization)); } catch (error) { if (!force && /授权.*失效|会话.*失效/.test(error.message || '')) return authorizeCurrentOrders(true); throw error; } renderOrdersList(); return state.orderAuthorization; }
async function loadOrders() { if (state.ordersLoading) return state.ordersLoading; state.ordersLoading = (async () => { $('#ordersMessage').textContent = ''; $('#ordersList').innerHTML = '<div class="skeleton"></div>'; try { await authorizeCurrentOrders(); } catch (error) { $('#ordersList').innerHTML = ''; $('#ordersMessage').textContent = error.message; } finally { state.ordersLoading = null; } })(); return state.ordersLoading; }
...
async function orderDetailAuthorization(tradeNo, force = false) { await authorizeCurrentOrders(force); return authorizeOrder(tradeNo); }
async function showOrderDetail(tradeNo, queryPassword = '', refreshCount = 0, preferPayment = true) { clearTimeout(state.orderDetailTimer); state.orderDetailTimer = null; $('#paymentModal').classList.add('hidden'); showView(true, false); if (!refreshCount) $('#orderDetail').innerHTML = '<p class="muted">正在查询订单…</p>'; $('#orderModal').classList.remove('hidden'); try { let paymentId = preferPayment ? state.paymentSessions.get(String(tradeNo)) : ''; let authorization = paymentId ? null : await orderDetailAuthorization(tradeNo); let data; try { data = await api('order-info', { trade_no: tradeNo, query_password: queryPassword, payment_id: paymentId || '', authorization_id: authorization?.authorization_id || '' }); } catch (error) { if (paymentId && /支付会话.*失效/.test(error.message || '')) { removeOrderSession(tradeNo); paymentId = ''; authorization = await orderDetailAuthorization(tradeNo); data = await api('order-info', { trade_no: tradeNo, query_password: queryPassword, authorization_id: authorization.authorization_id }); } else if (!paymentId && /(?:授权|会话).*失效/.test(error.message || '')) { authorization = await orderDetailAuthorization(tradeNo, true); data = await api('order-info', { trade_no: tradeNo, query_password: queryPassword, authorization_id: authorization.authorization_id }); } else throw error; } const cards = cardValues(data); const amount = data.total_amount ?? data.amount ?? data.price; const refreshing = !cards.length && isPaid(data) && refreshCount < 40; $('#orderDetail').innerHTML = `<dl class="order-fields"><div><dt>订单号</dt><dd>${escapeHtml(orderNo(data, tradeNo))}</dd></div><div><dt>商品</dt><dd>${escapeHtml(data.goods_name || data.name || data.title || '—')}</dd></div><div><dt>状态</dt><dd>${escapeHtml(orderStatus(data))}</dd></div>${amount !== undefined ? `<div><dt>金额</dt><dd>￥${Number(amount).toFixed(2)}</dd></div>` : ''}</dl><section class="cards-detail"><div class="cards-heading"><h3>卡密详情</h3>${cards.length > 1 ? `<button class="copy-card copy-all" type="button">复制全部</button>` : ''}</div>${cards.length ? cards.map((card, index) => `<article class="card-code"><div class="card-code-head"><span class="card-index">卡密 ${String(index + 1).padStart(2, '0')}</span><button class="copy-card" data-card="${escapeHtml(card)}" type="button">复制此卡密</button></div><code title="点击或长按可选择卡密">${escapeHtml(card)}</code></article>`).join('') : `<p class="muted">${refreshing ? '卡密正在发放，3 秒后自动刷新…' : '卡密暂未发放，请稍后重新打开订单详情。'}</p>`}</section><section class="order-instructions"><h3>订单使用说明</h3><div>${escapeHtml(String(data.instructions || '').replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '').trim() || '暂无使用说明')}</div></section>`; if (refreshing && !$('#orderModal').classList.contains('hidden')) state.orderDetailTimer = setTimeout(() => showOrderDetail(tradeNo, queryPassword, refreshCount + 1, Boolean(paymentId)), 3000); } catch (error) { $('#orderDetail').innerHTML = `<p class="message">${escapeHtml(error.message)}</p>`; } }
...
$('#showUserManagement').addEventListener('click', () => { $('#currentAccountPhone').textContent = state.account || '—'; $('#userManagementModal').classList.remove('hidden'); }); $('#closeUserManagement').addEventListener('click', () => $('#userManagementModal').classList.add('hidden')); $('#userManagementModal').addEventListener('click', (event) => { if (event.target === $('#userManagementModal')) $('#closeUserManagement').click(); }); $('#logoutAccount').addEventListener('click', logoutAccount); $('#accountHistoryList').addEventListener('click', (event) => { const button = event.target.closest('.history-account'); if (!button) return; saveAccount(button.dataset.phone); applyAccount(button.dataset.phone); });
