...
test('health、静态页可用且不存在网页重置路由', async () => {
  ...
  try { const health = await fetch(base + '/api/health').then(v => v.json()); assert.equal(health.code, 1); assert.equal(health.data.environment.shop_url, config.shopUrl); const page = await fetch(base + '/').then(v => v.text()); assert.match(page, /<title>/); assert.match(page, /id="shopView"/); const reset = await fetch(base + '/shop-pro/reset'); assert.equal(reset.status, 404); } finally { server.close(); }
});
